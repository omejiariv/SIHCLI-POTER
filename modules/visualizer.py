    ...
    # MODO 2: CUENCA
        ...
        if sel_cuencas:
            ...
            if res and res.get('ready'):
                ...
                st.markdown("---"); st.subheader("🌡️ Índices Climáticos")
                idx = res.get('idx', {})
                i1, i2 = st.columns(2)
                ...
                # D. Curvas FDC
                if res.get('fdc'):
                    st.markdown("---"); st.subheader("📉 Curva de Duración de Caudales (FDC)")
                    df_fdc = res['fdc']['data']
        ...
        if json_plumas:
            ...
            if data_plume:
                ...
                for model in data_plume['models']:
                    ...
                    color = "rgba(100, 149, 237, 0.6)" if is_dyn else "rgba(255, 165, 0, 0.6)" # Azul/Naranja
                    
                    # Nombre genérico para la leyenda
                    legend_group = "Modelos Dinámicos" if is_dyn else "Modelos Estadísticos"
                ...
                try:
                    ...
                    fig_plume.add_trace(go.Scatter(
                        x=data_plume['seasons'], y=avg_vals,
                        mode='lines+markers', name='PROMEDIO MULTIMODELO',
                        line=dict(color="black", width=4), marker=dict(size=6, color="black"),
                        showlegend=True
                    ))
                except Exception as e:
                ...
                # Umbrales
                fig_plume.add_hline(y=0.5, line_dash="dash", line_color="red", annotation_text="El Niño (+0.5)")
                fig_plume.add_hline(y=-0.5, line_dash="dash", line_color="blue", annotation_text="La Niña (-0.5)")
    ...
    with tab_iri_probs:
        if json_probs:
            ...
            try:
                ...
                meses = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic']
                st.info(f"📅 Pronóstico de Probabilidades (Consenso CPC/IRI) actualizado a: **{meses[last_month_idx]} {last_year}**")
            except:
                pass
...
def display_trends_and_forecast_tab(**kwargs):
    ...
    try:
        if mode_fc == "Estación Individual":
            ...
            if selected_station:
                ...
                full_idx = pd.date_range(start=station_data.index.min(), end=station_data.index.max(), freq='MS')
                ts_clean = station_data[Config.PRECIPITATION_COL].reindex(full_idx).interpolate(method='time').dropna()
            ...
            full_idx = pd.date_range(start=reg_data.index.min(), end=reg_data.index.max(), freq='MS')
    ...
    with tabs[1]:
        try:
            ...
            st.plotly_chart(fig)
        except: st.warning("Error en descomposición (datos insuficientes o discontinuos).")
    ...
    with tabs[2]:
        try:
            ...
            c1, c2 = st.columns(2)
            c1.plotly_chart(px.bar(x=range(len(lag_acf)), y=lag_acf, title="ACF (Autocorrelación)"))
            c2.plotly_chart(px.bar(x=range(len(lag_pacf)), y=lag_pacf, title="PACF (Parcial)"))
        except: pass

    # --- TAB 4: SARIMA ---
    with tabs[3]:
        st.markdown("#### Pronóstico SARIMA")
        sel_regs = st.multiselect("Usar Regresor Externo (ONI/SOI/IOD):", avail_regs, key="sarima_regs_sel")
        
        final_reg_df = None
        if sel_regs and regressors_df is not None:
            final_reg_df = regressors_df[sel_regs].reindex(ts_clean.index).fillna(method='ffill').fillna(method='bfill')
        ...
        if st.button("Calcular SARIMA"):
            ...
            with st.spinner("Calculando SARIMA..."):
                try:
                    ...
                    _, fc, ci, met, _ = generate_sarima_forecast(ts_in, order=(1,1,1), seasonal_order=(1,1,1,12), horizon=horizon, test_size=t_size, regressors=final_reg_df)
                    ...
                    if not ci.empty:
                        fig.add_trace(go.Scatter(x=pd.concat([pd.Series(ci.index), pd.Series(ci.index)[::-1]]), y=pd.concat([ci.iloc[:, 0], ci.iloc[:, 1][::-1]]), fill='toself', fillcolor='rgba(255,0,0,0.1)', line=dict(color='rgba(255,255,255,0)'), name='Confianza 95%'))
                    st.plotly_chart(fig)
    ...
    with tabs[4]:
        ...
        if sel_regs_p and regressors_df is not None:
            try:
                ...
                if 'ds' not in final_reg_p.columns and 'date' in final_reg_p.columns: final_reg_p.rename(columns={'date': 'ds'}, inplace=True)
                elif 'ds' not in final_reg_p.columns: final_reg_p.rename(columns={final_reg_p.columns[0]: 'ds'}, inplace=True)
            except Exception as e: 
        ...
        if st.button("Calcular Prophet"):
            ...
            with st.spinner("Calculando Prophet..."):
                try:
                    ...
                    fig.add_trace(go.Scatter(x=pd.concat([fc['ds'], fc['ds'][::-1]]), y=pd.concat([fc['yhat_upper'], fc['yhat_lower'][::-1]]), fill='toself', fillcolor='rgba(0,255,0,0.1)', line=dict(color='rgba(255,255,255,0)'), name='Confianza'))
                    st.plotly_chart(fig)
                    st.session_state['prophet_res'] = fc[['ds', 'yhat']].set_index('ds')['yhat']
                except Exception as e: st.error(f"Error Prophet: {e}")
        ...
        if df_anual is None or gdf_stations is None:
            ...
            try:
                ...
                def get_zone_data(row):
                    z_id = lz.classify_life_zone_alt_ppt(row[Config.ALTITUDE_COL], row[Config.PRECIPITATION_COL])
                    return pd.Series([lz.holdridge_int_to_name_simplified.get(z_id, "Desconocido"), lz.holdridge_colors.get(z_id, "#808080")])
